...
export MODEL_NAME="$DIR/checkpoints/Wan2.1-Fun-V1.1-1.3B-InP"

DATA_DIR="/data/dataset/talking_face_data"

CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch "$DIR/train_1B_rec_vec_lora.py" \
  ...
  --train_data_rec_dir="$DATA_DIR/video_rec_path.txt" \
  --train_data_vec_dir="$DATA_DIR/video_vec_path.txt"  \
  ...
  --lr_warmup_steps=100 \
  --seed=42 \
  --output_dir="$DIR/output_1B_rec_vec_lora_dir" \
